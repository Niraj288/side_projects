...
def process(X,size,d3=0):
	...
	for i in range (len(X)):
		X_new.append(preprocessing_3d.process3(X[i],[a,b],d3))
...
def get_Xy(d,dic):
	...
	for ab in d['FD']:
		nuc=[]
		for j in dk['X'][ref]:
			nuc.append(0.5*j[0]**2)
		x=ab['PE']+nuc
		if len(x)>t:
			print len(x)
		#x=x+list(Xk[ref])+[0.0]*(2500-len(x))
		x=x+[0.0]*(t-len(x))
		x.sort(reverse=1)
		X.append(x)
		ref+=1
...
def run():
	#d={'X_processed':X,'y':y}
	d=np.load('katja_integrals.npy').item()
	#d=np.load('/users/nirajv/data/katja_integrals.npy').item()
	dic=read_inp()
	print dic 
	#X,y=get_Xy()
	#d['X']=X 
	#np.save('katja_data.npy',d)
	X,y=get_Xy(d,dic),d['y']
